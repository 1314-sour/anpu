<template>
  <div class="page-content">
    <div class="page-header">全部消息</div>
    <div class="page-body">
      ...
      <el-table
        ...
        <el-table-column prop="title" label="消息标题" width="700">
          <template slot-scope="scope">
            <span class="message-dot" v-if="scope.row.status === '未读'">•</span>
            {{ scope.row.title }}
          </template>
        </el-table-column>
        <el-table-column prop="status" label="状态" width="400">
          <template slot-scope="scope">
            <span :class="scope.row.status === '未读' ? 'text-danger' : 'text-info'">{{ scope.row.status }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="time" label="发布时间" width="500"></el-table-column>
        <el-table-column prop="type" label="类型" width="500"></el-table-column>
