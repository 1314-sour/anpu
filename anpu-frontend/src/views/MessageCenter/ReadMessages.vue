<template>
  <div class="page-content">
    <div class="page-header">已读消息</div>
    <div class="page-body">
      ...
      <div class="action-bar">
        <div class="left-actions">
          ...
          <el-button size="small" @click="handleMarkUnread" :disabled="selectedMessages.length === 0">标记未读</el-button>
      ...
      <el-table
        ...
        <el-table-column prop="title" label="消息标题" width="400">
          <template slot-scope="scope">
            <span class="text-info">{{ scope.row.title }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="状态" width="700">
          <template slot-scope="scope">
            <span class="text-info">{{ scope.row.status }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="time" label="发布时间" width="500">
           <template slot-scope="scope">
            <span class="text-info">{{ scope.row.time }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="type" label="类型" width="500">
           <template slot-scope="scope">
            <span class="text-info">{{ scope.row.type }}</span>
          </template>
        </el-table-column>
...
<script>
import { getMessages, deleteMessages, markUnread } from '@/api/message'
...
export default {
  ...
  methods: {
    ...
    async handleMarkUnread() {
      ...
      try {
        await markUnread(this.selectedMessages.map(m => m.id))
